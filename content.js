...
async function craftPrompt() {
  ...
  const aspectHelpers = {
    ...
    shot: "Describe specific camera angle, movement style/tripod static, and framing in 5-7 phrases.",
    action: "Outline key movements and events in the scene in 5-7 phrases."
  };

  const dynamicAspectHelpers = uncheckedSections.map(section => `- ${section.charAt(0).toUpperCase() + section.slice(1)}: ${aspectHelpers[section]}`).join('\n');

  const systemPrompt = `You are a video description assistant specializing in concise yet detailed visual descriptions. Given a general prompt, provide specific, full details for the following aspects of one short shot (5 seconds) of a film: ${uncheckedSections.join(', ')}. Focus on creating rich visual imagery using short, comma-separated phrases. For each aspect:
${dynamicAspectHelpers}
